...
const MasterIndustry = db.masterIndustry;
...
exports.findAll = (req, res) => {
  ...
  MasterIndustry.findAndCountAll({
...
exports.findAllPublished = (req, res) => {
  ...
  MasterIndustry.findAndCountAll({ where: { published: true }, limit, offset })
    .then(data => {
      const response = functions.getPagingData(data, page, limit);
      res.send(response);
  })
...
exports.createOne = (req, res) => {
  ...
  MasterIndustry.create({title, published})
  .then(data=>{
    res.status(200).send({
      code: 200,
      success: true,
...
exports.update = (req, res) => {
  ...
  MasterIndustry.update(req.body, {where: {id: id}})
  .then(data=>{
    if(data.length == 0){
      res.status(200).send({
        code: 200,
